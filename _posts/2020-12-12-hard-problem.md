...
4. <a name="r3"></a>A modern attempt to sidestep this issue contends that we don't have sense impressions in the sense of there being some special phenomenal entities which mediate access to non-phenomenal entities, but rather that phenomenal _properties_ accompany our unmediated access to non-phenomenal entities. However, this only seems to make the reference-fixing problem worse, since we have access to properties only through their bearers.
